...
**LixEditor** is the block-based WYSIWYG editor that powers LixSketch. It is built on BlockNote + React, ships a custom block schema (image, equation, mentions, tabs, table of contents), and is real-time-collaboration ready via Yjs. This page documents the developer API.
...
Persist the `Block[]` JSON from `onChange`, then render it read-only later with `editable={false}` and `initialContent={blocks}` — the same renderer LixSketch uses for published posts.
