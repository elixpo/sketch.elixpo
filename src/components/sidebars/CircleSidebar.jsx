        ...
        <div className="flex flex-col gap-0.5">
          {FILLS.map((f) => (
            <button key={f.value} onClick={() => updateFill(f.value)}
              className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs transition-all duration-100 ${fillStyle === f.value ? 'bg-[#6f846f] text-white' : 'text-text-secondary hover:bg-surface-hover'}`}
            >
