        ...
        <div className="flex items-center gap-1">
          {[1, 2, 4, 7].map((w) => (
            <button key={w} onClick={() => updateThickness(w)}
              className={`w-9 h-8 flex items-center justify-center rounded-lg transition-all duration-100 ${thickness === w ? 'bg-[#7667a8]/20 text-[#7667a8]' : 'text-text-muted hover:bg-surface-hover'}`}
            >
        ...
        <div className="flex items-center gap-1">
          {[{ v: 0, l: '0' }, { v: 2, l: '2' }, { v: 4, l: '4' }].map((s) => (
            <button key={s.v} onClick={() => updateSloppiness(s.v)}
              className={`w-8 h-8 flex items-center justify-center rounded-lg text-xs transition-all duration-100 ${sloppiness === s.v ? 'bg-[#7667a8]/20 text-accent-blue' : 'text-text-muted hover:bg-surface-hover'}`}
            >{s.l}</button>
          ))}
        </div>
      </ToolbarButton>
        ...
        <div className="flex flex-col gap-0.5">
          {[{ v: 'smooth', i: 'bxs-droplet', l: 'Smooth' }, { v: 'rough', i: 'bxs-bolt', l: 'Rough' }].map((e) => (
            <button key={e.v} onClick={() => updateEdge(e.v)}
              className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs transition-all duration-100 ${edge === e.v ? 'bg-accent-blue text-white' : 'text-text-secondary hover:bg-surface-hover'}`}
            >
