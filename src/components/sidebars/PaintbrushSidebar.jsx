    ...
    <ShapeSidebar visible={activeTool === TOOLS.FREEHAND || selectedShapeSidebar === 'paintbrush'}>
        ...
        <div className="flex flex-col gap-0.5">
          {[
            ...
            <button key={t.v} onClick={() => updateTaper(t.v)}
              className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs transition-all duration-100 ${taper === t.v ? 'bg-[#6f846f] text-white' : 'text-text-secondary hover:bg-surface-hover'}`}
            >
        ...
        <div className="flex flex-col gap-0.5">
          {[
            ...
            <button key={r.v} onClick={() => updateRoughness(r.v)}
              className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs transition-all duration-100 ${roughness === r.v ? 'bg-[#6f846f] text-white' : 'text-text-secondary hover:bg-surface-hover'}`}
            >
      ...
      <ToolbarButton icon="bxs-sun" tooltip={t('sidebar.opacity')}>
        ...
        <input
          ...
          className="w-28 h-1 bg-white/10 rounded-full appearance-none cursor-pointer accent-[#6f846f]"
        />
      </ToolbarButton>
      <Divider />
      <LayerControls />
