...
export default function PaintBucketSidebar() {
  ...
  return (
    <ShapeSidebar visible={activeTool === TOOLS.PAINT_BUCKET}>
      <ToolbarButton
        tooltip="Fill color"
        preview={<span className="h-4 w-4 rounded-md border border-border-light" style={{ backgroundColor: fillColor }} />}
      >
        ...
        <div className="grid grid-cols-4 gap-1.5">
          {COLORS.map((color) => (
            <button
              ...
              className={`h-7 w-7 cursor-pointer rounded-md border-[1.5px] transition ${fillColor === color ? 'scale-110 border-accent' : 'border-border-light hover:border-text-dim'}`}
        ...
        <div className="flex flex-col gap-0.5">
          {FILLS.map((fill) => (
            <button
              ...
              className={`flex cursor-pointer items-center gap-2 rounded-lg px-2.5 py-1.5 text-xs transition ${fillStyle === fill.value ? 'bg-accent text-white' : 'text-text-secondary hover:bg-surface-hover'}`}
            >
