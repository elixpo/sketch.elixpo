...
export default function CanvasRulers({ enabled, svgRef }) {
    ...
    const sync = () => {
      ...
      if (svg) {
        ...
        const next = {
          ...
          width: rect.width,
          height: rect.height,
          rulerLeft: Math.max(0, Math.min(
            hostRect.width - RULER_SIZE,
            toolbarRect ? toolbarRect.right - hostRect.left + 8 : 64,
          )),
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-[900] select-none text-text-muted" aria-hidden="true">
      <div
        className="absolute right-0 overflow-hidden border-b border-border-light bg-surface/95 shadow-sm backdrop-blur-sm"
        style={{ height: RULER_SIZE, left: horizontalStart, top: viewport.rulerTop }}
      >
      ...
      <div
        ...
        style={{ width: RULER_SIZE, left: viewport.rulerLeft, top: verticalStart }}
      >
