...
export default function WebEmbedModal() {
  ...
  const submit = (e) => {
    ...
    if (!normalized) { setError('This URL is not allowed for embeds yet. Raise an issue to request support.'); return; }
    window.__pendingWebEmbedURL = normalized; setOpen(false); setUrl('');
  };
  return <div className="fixed inset-0 z-[12000] flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm">
    <form onSubmit={submit} className="w-full max-w-xl rounded-2xl border border-border-light bg-surface-card p-6 font-[lixFont] shadow-2xl">
      ...
      <button type="submit" className="mt-5 w-full rounded-xl bg-accent px-4 py-3 text-sm text-white">Continue to canvas</button>
    </form>
  </div>;
}
