...
export default function Toolbar() {
  ...
  const moreActive = moreItems.some((item) => item.tool === activeTool)
              ...
              {moreOpen && (
                <div className="absolute left-[calc(100%+10px)] top-1/2 z-[1100] w-[190px] -translate-y-1/2 rounded-xl border border-border-light bg-surface-card/95 p-1.5 shadow-xl shadow-black/25 backdrop-blur-lg">
                  ...
                  {moreItems.map((moreItem) => {
                    ...
                    return (
                      <button key={moreItem.tool} type="button" title={`${moreItem.title} (${moreItem.key})`} onClick={() => { setActiveTool(moreItem.tool); setMoreOpen(false) }} className={`flex w-full cursor-pointer items-center gap-3 rounded-lg px-2.5 py-2 text-left text-xs transition ${selected ? 'bg-accent/20 text-accent' : 'text-text-muted hover:bg-surface-hover hover:text-text-primary'}`}>
